...
export default async function RepositoryPage({ params }: { params: Promise<{ owner: string; repo: string }> }) {
  ...
  return (
    <main className="detail-shell" id="main" tabIndex={-1}>
      <Link className="back-link" href="/#registry">← DEEP REGISTRY</Link>
      <section className="detail-hero"><div><span className="eyebrow">Verified public repository</span><h1>{owner}/{repo}</h1><p>{repository.skills.length} indexed public skill{repository.skills.length === 1 ? "" : "s"} descend from this source.</p></div></section>
