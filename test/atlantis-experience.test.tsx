...
describe("Atlantis canonical experience", () => {
  beforeEach(() => {
  ...
  it("shows the portal once and records completion in session storage", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /Enter Atlantis/i }));
    expect(sessionStorage.getItem("atlantis.portalIntroSeen.v1")).toBe("true");
  ...
  it("skips the automatic entrance after it has been seen and supports replay", async () => {
    sessionStorage.setItem("atlantis.portalIntroSeen.v1", "true");
    render(<PortalEntrance />);
    await waitFor(() => expect(screen.queryByRole("dialog")).not.toBeInTheDocument());
    act(() => window.dispatchEvent(new Event("atlantis:open-gate")));
    expect(screen.getByRole("dialog", { name: /Every update is/i })).toBeInTheDocument();
  });
  ...
  it("lets visitors pause the real public update ticker", () => {
    render(<AtlantisTicker skills={demoCatalog.skills} />);
